...
class Stopwatch : SurfaceView, SurfaceHolder.Callback {
    ...
    private val onContextLifecycleListener = object : Application.ActivityLifecycleCallbacks {
        ...
        override fun onActivityResumed(activity: Activity) {
            if (activity.findViewById<View>(android.R.id.content).tag == tag) {
                onResume()
            }
        }
    ...
    private fun initView(context: Context, attrs: AttributeSet?, defStyleAttr: Int) {
        if (context is Activity) {
            context.findViewById<View>(android.R.id.content).tag = tag
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        when (widthMode) {
            ...
            MeasureSpec.AT_MOST -> {
                finalWidth = widthArgs * sp2px(context, txtSize).toInt()
            }
